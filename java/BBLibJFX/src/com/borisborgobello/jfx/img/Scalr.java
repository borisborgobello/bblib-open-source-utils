...
import static com.borisborgobello.jfx.img.BBImgUtils.createBI;
...
public class Scalr {
    ...
    if (DEBUG)
      t = System.currentTimeMillis();
		...
		if (colorHasAlpha || imageHasAlpha) {
			...
			result = createBI(newWidth, newHeight,
					true);
		} else {
			if (DEBUG)
				log(1,
		...
		if (width <= 0 || height <= 0)
			throw new IllegalArgumentException("width [" + width
                ...
                try {
                    ...
                    img = BBImgUtils.BigBufferedImage.create(width,height,type);
                }
		return img;
	}
	...
	protected static BufferedImage copyToOptimalImage(BufferedImage src)
		...
		int type = (src.getTransparency() == Transparency.OPAQUE ? BufferedImage.TYPE_INT_RGB
				: BufferedImage.TYPE_INT_ARGB);
		BufferedImage result = createBI(src.getWidth(),
				src.getHeight(), type == BufferedImage.TYPE_INT_ARGB);
